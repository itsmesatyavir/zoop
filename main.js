...
const BANNER = `
==========================================
  Zoop Auto Spin + Daily Checkin Bot (https://t.me/forestarmy)
==========================================
`;
...
async function getAccessToken(queryId, proxyAgent) {
  try {
...
async function checkUserInfo(token, userId, proxyAgent) {
  try {
    ...
    return {
      spinAvailable: response.data.data.spined === false,
      dailyClaimed: response.data.data.claimed,
      dayClaim: response.data.data.dayClaim
    };
  } catch (error) {
    logMessage(`Error checking user info: ${error.response?.data?.message || error.message}`);
    throw error;
  }
}
...
async function runBot() {
  ...
  try {
    ...
    const token = await executeWithRetry(getAccessToken, queryId, proxyAgent);
    
    while (true) {
      const userInfo = await executeWithRetry(checkUserInfo, token, userId, proxyAgent);
      
      if (!userInfo.dailyClaimed) {
        await executeWithRetry(claimDailyTask, token, userId, proxyAgent);
      } else {
        logMessage("Daily task already claimed today");
      }
      
      if (userInfo.spinAvailable) {
